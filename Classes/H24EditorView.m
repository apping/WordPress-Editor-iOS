...
typedef void(^H24EditorViewCallbackParameterProcessingBlock)(NSString* parameterName, NSString* parameterValue);
typedef void(^H24EditorViewNoParamsCompletionBlock)();
...
static NSString* const kH24EditorViewFieldTitleId = @"zss_field_title";
static NSString* const kH24EditorViewFieldContentId = @"zss_field_content";
...
static NSString* const H24EditorViewWebViewContentSizeKey = @"contentSize";

@interface H24EditorView () <UITextViewDelegate, UIWebViewDelegate, UITextFieldDelegate>
...
#pragma mark - Fields
@property (nonatomic, weak, readwrite) H24EditorField* focusedField;

@end

@implementation H24EditorView
...
- (void)createSourceTitleViewWithFrame:(CGRect)frame
{
    ...
    _sourceViewTitleField.font = [H24FontManager openSansBoldFontOfSize:24.0];
...
- (void)createSourceDividerViewWithFrame:(CGRect)frame
{
    ...
    _sourceContentDividerView.backgroundColor = [UIColor darkGrayColor];
...
{
    NSBundle * bundle = [NSBundle bundleForClass:[H24EditorView class]];
                       ...
                       context:(void *)context
{
    ...
    if (object == self.webView.scrollView) {
        ...
        if ([keyPath isEqualToString:H24EditorViewWebViewContentSizeKey]) {
...
- (void)startObservingWebViewContentSizeChanges
{
    [_webView.scrollView addObserver:self
                          forKeyPath:H24EditorViewWebViewContentSizeKey
...
- (BOOL)handleWebViewCallbackURL:(NSURL*)url
{
	...
	LogDebug(@"WebEditor callback received: %@", url);
    ...
    [self parseParametersFromCallbackURL:url
         andExecuteBlockForEachParameter:^(NSString *parameterName, NSString *parameterValue)
     {
         if ([parameterName isEqualToString:kFieldIdParameterName]) {
             if ([parameterValue isEqualToString:kH24EditorViewFieldTitleId]) {
                 self.focusedField = self.titleField;
             } else if ([parameterValue isEqualToString:kH24EditorViewFieldContentId]) {
    ...
    [self parseParametersFromCallbackURL:url
         andExecuteBlockForEachParameter:^(NSString *parameterName, NSString *parameterValue)
     {
         ...
         if ([self.delegate respondsToSelector:@selector(editorView:imageTapped:url:imageMeta:)]) {
             H24ImageMeta *imageMeta = [H24ImageMeta imageMetaFromJSONString:tappedMeta];
    ...
    [self parseParametersFromCallbackURL:url
         andExecuteBlockForEachParameter:^(NSString *parameterName, NSString *parameterValue)
     {
         if ([parameterName isEqualToString:kMessageParameterName]) {
             LogInfo(@"WebEditor log:%@", [self stringByDecodingURLFormat:parameterValue]);
         }
     } onComplete:nil];
}
...
- (void)handleNewFieldCallback:(NSURL*)url
{
    ...
    [self parseParametersFromCallbackURL:url
         andExecuteBlockForEachParameter:^(NSString *parameterName, NSString *parameterValue)
     {
         ...
         H24EditorField* newField = [self createFieldWithId:fieldId];
...
- (void)parseParametersFromCallbackURL:(NSURL*)url
	   andExecuteBlockForEachParameter:(H24EditorViewCallbackParameterProcessingBlock)block
							onComplete:(H24EditorViewNoParamsCompletionBlock)onCompleteBlock
{
	NSParameterAssert([url isKindOfClass:[NSURL class]]);
 ...
 *  @todo       At some point it would be nice to have H24EditorView be able to handle a custom list
 ...
 */
- (H24EditorField*)createFieldWithId:(NSString*)fieldId
{
    NSAssert([fieldId isKindOfClass:[NSString class]],
             @"We're expecting a non-nil NSString object here.");
    
    H24EditorField* newField = nil;
    
    if ([fieldId isEqualToString:kH24EditorViewFieldTitleId]) {
        NSAssert(!_titleField,
                 @"We should never have to set this twice.");
        
        _titleField = [[H24EditorField alloc] initWithId:fieldId webView:self.webView];
        newField = self.titleField;
    } else if ([fieldId isEqualToString:kH24EditorViewFieldContentId]) {
        NSAssert(!_contentField,
                 @"We should never have to set this twice.");
        
        _contentField = [[H24EditorField alloc] initWithId:fieldId webView:self.webView];
        newField = self.contentField;
    }
    NSAssert([newField isKindOfClass:[H24EditorField class]],
...
- (void)updateCurrentImageMeta:(H24ImageMeta *)imageMeta
{
 ...
 */
- (void)callDelegateFieldCreated:(H24EditorField*)field
{
    NSParameterAssert([field isKindOfClass:[H24EditorField class]]);
 ...
 */
- (void)callDelegateFieldFocused:(H24EditorField*)field
{
